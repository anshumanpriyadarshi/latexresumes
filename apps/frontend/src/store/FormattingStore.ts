...
export class FormattingStore {
  ...
  updateSectionSpacing = action(
    (section: SectionKey, key: keyof SpacingConfig, value: LaTeXUnit | undefined): void => {
      if (!this.formatting) return;
      const sec = this.formatting.sections[section] ?? (this.formatting.sections[section] = {});
  ...
  updateSectionFont = action(
    (section: SectionKey, key: keyof FontConfig, value: FontDef | undefined): void => {
      if (!this.formatting) return;
      const sec = this.formatting.sections[section] ?? (this.formatting.sections[section] = {});
      if (!sec.fonts) sec.fonts = {};
      if (value === undefined) {
        delete sec.fonts[key];
      } else {
        sec.fonts[key] = value;
      }
      this.touch();
  ...
  updateSectionAlignment = action(
    (section: SectionKey, key: keyof AlignmentDef, value: AlignmentDef[keyof AlignmentDef] | undefined): void => {
      if (!this.formatting) return;
      const sec = this.formatting.sections[section] ?? (this.formatting.sections[section] = {});
      if (!sec.alignment) sec.alignment = {};
      if (value === undefined) {
        if (key === 'vertical') {
          delete sec.alignment.vertical;
        } else {
          delete sec.alignment.horizontal;
        }
      } else {
        if (key === 'vertical') {
          sec.alignment.vertical = value as AlignmentDef['vertical'];
        } else {
          sec.alignment.horizontal = value as AlignmentDef['horizontal'];
        }
      }
      this.touch();
    }
  ...
  applyTheme = action((theme: Theme): void => {
    if (!this.formatting) return;
    Object.assign(this.formatting, JSON.parse(JSON.stringify(theme.formatting)));
    this.touch();
  });
  ...
  exportTheme = (name: string): Theme => ({
    ...
    formatting: JSON.parse(JSON.stringify(this.formatting ?? this.activeFormatting)) as Theme['formatting'],
  });
}
